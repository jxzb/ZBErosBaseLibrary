...
//    }];
    ...
    __weak typeof(self)weakSelf = self;
    LocalPhotoController *imagePickerVc = [[LocalPhotoController alloc]init];
    imagePickerVc.locationStr = self.imageInfo.locationStr;
    imagePickerVc.outLine = self.imageInfo.outLine;
    if (!self.imageInfo.outLine) {
        imagePickerVc.count = self.imageInfo.maxCount;
    }
    imagePickerVc.userName = self.imageInfo.userName;
    imagePickerVc.doneImageBlock = ^(NSArray *imgArr) {
        NSLog(@"%@",imgArr);
        NSMutableArray *photts = [NSMutableArray new];
        for (NSDictionary *dic in imgArr) {
            [photts addObject:dic[@"photoImg"]];
        }
        
        if (weakSelf.isLocal) {
            [weakSelf cacheImages:photts infoData:imgArr];
        } else {
            [weakSelf uploadImage:photts];
        }
    };
...
- (void)cacheImages:(NSArray *)images infoData: (NSArray *)infoArr
{
    @weakify(self);
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        @strongify(self);
        ...
        for (int i = 0; i < infoArr.count ; i ++) {
            NSString *path = [self saveImage2Disk:infoArr[i][@"photoImg"]];
            [infoArr[i] setObject:path forKey:@"photoImg"];
        }
        NSLog(@"%@",infoArr);
        
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self.callback) {
                NSDictionary *backData = [NSDictionary configCallbackDataWithResCode:BMResCodeSuccess msg:nil data:infoArr];
                self.callback(backData);
            }
        });
    });
